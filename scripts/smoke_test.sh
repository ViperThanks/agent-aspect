...
HOME_OVERRIDE="/tmp/agent-aspect-smoke-home-$$"
mkdir -p "$HOME_OVERRIDE/.agent-aspect"
export HOME="$HOME_OVERRIDE"
export AGENT_ASPECT_ASSUME_NO_TTY=1
rm -f "$HOME_OVERRIDE/.agent-aspect/ipc.sock" "$HOME_OVERRIDE/.agent-aspect/audit.db" "$HOME_OVERRIDE/.agent-aspect/state.json" "$HOME_OVERRIDE/.agent-aspect/config.toml"
...
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Edit","tool_input":{"file_path":"/tmp/test/.env","old_string":"OLD","new_string":"NEW","replace_all":false}}' \
    | "$BIN/agent-aspect-hook" 2>/dev/null)
...
EVENTS=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT COUNT(*) FROM events;")
DECISIONS=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT COUNT(*) FROM decisions;")
...
RULE_ID=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT rule_id FROM decisions WHERE rule_id IS NOT NULL LIMIT 1;")
if [ -n "$RULE_ID" ]; then
...
STATE_FILE="$HOME_OVERRIDE/.agent-aspect/state.json"
...
CONFIG_FILE="$HOME_OVERRIDE/.agent-aspect/config.toml"
...
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"git push origin main"}}' \
    | "$BIN/agent-aspect-hook" --interactive 2>/dev/null)
...
kill "$DAEMON_PID" 2>/dev/null || true
sleep 0.5
rm -f "$HOME_OVERRIDE/.agent-aspect/ipc.sock" "$HOME_OVERRIDE/.agent-aspect/audit.db" "$HOME_OVERRIDE/.agent-aspect/state.json"

echo 'mode = "observer"' > "$HOME_OVERRIDE/.agent-aspect/config.toml"
...
OBS_NOTE=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT note FROM decisions WHERE note LIKE '%observer%' LIMIT 1;")
...
echo 'mode = "autonomous"' > "$HOME_OVERRIDE/.agent-aspect/config.toml"
...
echo 'mode = "paranoid"' > "$HOME_OVERRIDE/.agent-aspect/config.toml"
...
MODE_OUT=$("$BIN/agent-aspect" mode)
if [ "$MODE_OUT" = "paranoid" ]; then
    echo "PASS: agent-aspect mode reads paranoid from config.toml"
else
    echo "FAIL: expected paranoid, got: $MODE_OUT"
    FAILED=1
fi

"$BIN/agent-aspect" mode autonomous > /dev/null
MODE_OUT=$("$BIN/agent-aspect" mode)
if [ "$MODE_OUT" = "autonomous" ]; then
    echo "PASS: agent-aspect mode autonomous written and read back"
else
    echo "FAIL: expected autonomous after set, got: $MODE_OUT"
    FAILED=1
fi

"$BIN/agent-aspect" mode guard > /dev/null
MODE_OUT=$("$BIN/agent-aspect" mode)
if [ "$MODE_OUT" = "guard" ]; then
    echo "PASS: agent-aspect mode guard written and read back"
else
    echo "FAIL: expected guard after set, got: $MODE_OUT"
    FAILED=1
fi

if grep -q 'mode = "guard"' "$HOME_OVERRIDE/.agent-aspect/config.toml"; then
    echo "PASS: config.toml contains mode = \"guard\""
else
    echo "FAIL: config.toml does not have expected content"
    cat "$HOME_OVERRIDE/.agent-aspect/config.toml"
    FAILED=1
fi
...
"$BIN/agent-aspect" mode observer > /dev/null
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"git push origin main"}}' \
    | "$BIN/agent-aspect-hook" 2>/dev/null)
...
"$BIN/agent-aspect" mode paranoid > /dev/null
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"echo hello"}}' \
    | "$BIN/agent-aspect-hook" 2>/dev/null)
...
STATE_MODE=$(python3 -c "import json; print(json.load(open('$HOME_OVERRIDE/.agent-aspect/state.json'))['mode'])" 2>/dev/null || echo "")
...
KIMI_COUNT=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT COUNT(*) FROM events WHERE agent = 'kimi_code';")
CODEX_COUNT=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT COUNT(*) FROM events WHERE agent = 'codex_cli';")
...
echo ""
echo "=== test 50: AGENT_ASPECT_AGENT=kimi + Claude Bash payload → env override to kimi_code, deny D003 ==="
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"rm -rf /tmp/important"}}' \
    | AGENT_ASPECT_AGENT=kimi "$BIN/agent-aspect-hook" 2>/dev/null)
if echo "$RESP" | grep -q '"deny"'; then
    echo "PASS: AGENT_ASPECT_AGENT=kimi env override accepted, deny D003"
else
    echo "FAIL: expected deny with AGENT_ASPECT_AGENT=kimi, got: $RESP"
    FAILED=1
fi
LAST_AGENT=$(sqlite3 "$HOME_OVERRIDE/.agent-aspect/audit.db" "SELECT agent FROM events ORDER BY rowid DESC LIMIT 1;")
...
echo ""
echo "=== test 51: AGENT_ASPECT_AGENT=gemini + Claude Bash echo hello → stderr warning, allow ==="
RESP=$(echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"echo hello"}}' \
    | AGENT_ASPECT_AGENT=gemini "$BIN/agent-aspect-hook" 2>/tmp/smoke_test_gemini_stderr.txt)
if [ -z "$RESP" ]; then
    echo "PASS: AGENT_ASPECT_AGENT=gemini allows after fallback"
else
    echo "FAIL: expected allow for AGENT_ASPECT_AGENT=gemini, got: $RESP"
    FAILED=1
fi
