...
//! - shared_ui 模块：marked → view_model → render → api_client → job_body → job_status → provider_capabilities → runtime_health → activity_segment
...
const PROVIDER_CAPABILITIES_JS: &str = include_str!("../../shared_ui/provider_capabilities.js");
...
pub async fn serve_ui() -> Response {
    ...
    let js = MARKED_JS.to_string()
        ...
        + " build="
        + env!("BUILD_TIME")
        + " shell=relay loaded=marked,view_model,render,api_client,job_body,job_status,provider_capabilities,runtime_health,activity_segment,approval_review');\n";
